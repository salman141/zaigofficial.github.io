...
function tie_theme_update_db(){
	...
	# Debug
	//update_option( 'tie_ver_'. TIELABS_THEME_ID, '4.9.0' );
	...
	if( $current_version ){
		...
		if( version_compare( $current_version, TIELABS_DB_VERSION, '<' ) ){
			...
			if( version_compare( $current_version, '5.0.0', '<' ) ){
				...
				if( ! empty( $updated_options['share_center_top'] ) ) {
					$updated_options['share_position_top'] = 'center';
				}

				if( ! empty( $updated_options['share_center_bottom'] ) ) {
					$updated_options['share_position_bottom'] = 'center';
				}

				$updated_options['mobile_menu_search_position'] = 'bottom';

				if( ! empty( $updated_options['mobile_menu_active'] ) ){
					$updated_options['mobile_header_components_menu'] = 'area_1';
				}

				if( ! empty( $updated_options['mobile-components_login'] ) ){
					$updated_options['mobile_header_components_login'] = 'area_1';
				}

				if( ! empty( $updated_options['mobile-components_bp_notifications'] ) ){
					$updated_options['mobile_header_components_bp_notifications'] = 'area_1';
				}

				if( ! empty( $updated_options['mobile-components_cart'] ) ){
					$updated_options['mobile_header_components_cart'] = 'area_1';
				}
