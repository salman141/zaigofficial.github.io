(function ($) {
	...
	function valen_do_ajax($blog_layout){ 
	...
	function valen_auto_do_ajax($blog_layout){ 
		if( $('#navigation-ajax').length > 0 ){
		...
		if( $('#navigation-ajax').length > 0 ){
			...
			if ( $('#navigation-ajax').hasClass('click-load') ) {
				$('#navigation-ajax').on('click', function() {
					...
					valen_do_ajax($valen_blog_layout);
				...
				$(window).scroll(function() {
					...
					if ( navTop + $('#navigation-ajax').outerHeight() <= scrollBottom ) {
						setTimeout(function(){
							...
							valen_auto_do_ajax($valen_blog_layout);
						}, 800);
					}
				});
			}
