<?php
class valen_Megamenu_Front extends Walker_Nav_Menu { 
    var $columns = 0;
    var $enablemega = 0;
    var $usepostwcode = '';
    var $postwcode = '';
            ...
            if ( $this->postwcode !='' && $this->usepostwcode != '' ) {
                ...
                if ($m_wcode->have_posts()) {
                    ...
                    if ( $this->usepostwcode == 'left' ) {
                        $l_content = '<div class="mega-left-content">'. do_shortcode('[valen_postwcode name="'.esc_attr($this->postwcode).'"]') .'</div>';
                    }
                }
                wp_reset_postdata();
            }
            ...
            if ( $this->postwcode !='' && $this->usepostwcode != '' ) {
                ...
                if ($m_wcode->have_posts()) {
                    if ( $this->usepostwcode == 'right' ) {
                        $r_b_content = '<div class="mega-right-content">'.do_shortcode('[valen_postwcode name="'.esc_attr($this->postwcode).'"]').'</div>';
                    }elseif ( $this->usepostwcode == 'bottom' ) {
                        $r_b_content = '<div class="mega-bottom-content">'.do_shortcode('[valen_postwcode name="'.esc_attr($this->postwcode).'"]').'</div>';
                    }
                }
                wp_reset_postdata();
            }
