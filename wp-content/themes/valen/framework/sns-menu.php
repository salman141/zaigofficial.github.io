<?php
class valen_MegaMenu {
    function __construct() {
        add_filter( 'wp_setup_nav_menu_item', array( $this, 'valen_megamenuset' ) );
		add_action( 'wp_update_nav_menu_item', array( $this, 'valen_megamenusave'), 10, 3 );
		add_filter( 'wp_edit_nav_menu_walker', array( $this, 'valen_megamenuedit'), 10, 2 );
    }
    function valen_megamenu_asset(){
    	add_action( 'admin_print_footer_scripts', array( $this, 'valen_iconmegapicker' ), 99 );
		add_action( 'admin_print_styles', array( $this , 'valen_loadadmincss'));
		add_action( 'admin_print_scripts', array( $this , 'valen_loadadminjs'));
    }
    // Set value
	function valen_megamenuset ( $item ) {
		$this->valen_megamenu_asset();
		// For 1st level
	...
	// Save option to db	
    function valen_megamenusave( $menu_id, $menu_item_db_id, $args ) {
		// Enable
	...
	// Edit form
    function valen_megamenuedit($walker, $menu_id) {
	    return 'valen_Megamenu_Admin'; 
	}
	// Load css file
	function valen_loadadmincss(){
		wp_enqueue_style('thickbox');
		wp_enqueue_style('valen-admin-megamenu', VALEN_THEME_URI.'/assets/css/admin_megamenu.css', false, '1.0', 'all' );
	...
	// Load js file
	function valen_loadadminjs(){
		wp_enqueue_script('thickbox');
		wp_enqueue_script('valen-admin-megamenu', VALEN_THEME_URI . '/assets/js/sns-admin-megamenu.js', array('jquery'), '', true);
	}
	function valen_iconmegapicker(){
	    ...
	    if( file_exists( get_template_directory().'/assets/fonts/valen-flaticon/font/flaticon.css' ) ) {
			$content_flat = $wp_filesystem->get_contents(get_template_directory().'/assets/fonts/valen-flaticon/font/flaticon.css');
	    }
		...
		<div id="sns_iconmegapicker">
		    ...
		    <h4 class="icon-heading"><?php echo esc_html__('FontAwesome Icons', 'valen'); ?></h4>
		    <?php		
		    if( is_array($icon_fa ) && !empty($icon_fa)) {
	     	...
	     	<h4 class="icon-heading"><?php echo esc_html__('Flat Icons', 'valen'); ?></h4>
		    <?php		
...
// Init valen_MegaMenu
$sns_mega = new valen_MegaMenu();
require_once VALEN_THEME_DIR . '/framework/mega-menu/admin.php';
require_once VALEN_THEME_DIR . '/framework/mega-menu/frontend.php';
