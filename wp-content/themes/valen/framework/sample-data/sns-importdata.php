<?php
if ( ! function_exists( 'valen_importdata' ) ) {
	function valen_importdata(){
		$msg = '';
		...
		if( $datatype=='theme' ){
			...
			valen_import_themeoptions($option_data);
		}
		if( $datatype=='slider' ){
			if(!valen_import_revslider()){
		...
		if( $datatype=='widget' ){
	        ...
	        //ob_start();
			if ( !valen_import_widget($widget_data) ){
...
if ( ! function_exists( 'valen_import_themeoptions' ) ){
	function valen_import_themeoptions($option){
		$option = json_decode($option,true);
		update_option('valen_themeoptions',$option);
	}
}
if(!function_exists('valen_import_revslider')){
	function valen_import_revslider(){
...
if(!function_exists('valen_import_widget')){
	function valen_import_widget($import_array){
		...
		foreach ( $sidebars_data as $import_sidebar => $import_widgets ) :
			foreach ( $import_widgets as $import_widget ) :
				...
				if ( isset( $wp_registered_sidebars[$import_sidebar] ) ) :
					...
					$new_widget_name 		= valen_widget_name( $title, $index );
...
if(!function_exists('valen_widget_name')){
	function valen_widget_name($widget_name, $widget_index){
