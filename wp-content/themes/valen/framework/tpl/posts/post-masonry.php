	...
	<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	    ...
	    if ( get_post_format() == 'quote' && function_exists('rwmb_meta') && rwmb_meta('valen_post_quotecontent') && rwmb_meta('valen_post_quoteauthor') ) :
	        ...
	        <div class="quote-info quote-info-<?php echo esc_attr($uq); ?>">
	            ...
	            <div class="quote-content gfont"><i class="fa fa-quote-left"> </i> <?php echo esc_html(rwmb_meta('valen_post_quotecontent')); ?> <i class="fa fa-quote-right"></i></div>
	            <?php endif; ?>
	    ...
	    <?php
	    // Post Video
	    elseif ( get_post_format() == 'video' && function_exists('rwmb_meta') && rwmb_meta('valen_post_video') ) : ?>
	        <div class="video-thumb video-responsive">
	            <?php
	            echo rwmb_meta('valen_post_video');
	            ?>
	            
	        </div>
	    <?php
	    // Post audio
        elseif ( get_post_format() == 'audio' && function_exists('rwmb_meta') && rwmb_meta('valen_post_audio') ) : ?>
            <div class="audio-thumb audio-responsive">
                <?php
                echo wp_oembed_get(esc_attr(rwmb_meta('valen_post_audio')));
                ?>
            </div>
        <?php
	    // Post Gallery
	    elseif ( get_post_format() == 'gallery' && function_exists('rwmb_meta') && rwmb_meta('valen_post_gallery') ) : 
	    ?>
	    ...
	    elseif ( has_post_thumbnail() ) : ?>
	        <div class="post-thumb">
	            <?php
	            $blog_type = valen_themeoption('blog_type');
	            $img_size = 'valen_blog_masonry_thumb';
	            ?>
		            <a href="<?php the_permalink(); ?>" title="<?php echo esc_attr( sprintf( esc_html__( '%s', 'valen' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark">
		            <?php
	        ...
	        <div class="content">
	            <div class="post-meta">
	            	<?php if ( is_sticky() && ! is_paged() ) { ?>
		                <span class="sticky-content"><?php echo esc_html__( 'Featured', 'valen' ) ; ?></span>
		            <?php
		            } ?>
	                <?php if( valen_themeoption('show_author', 1) == 1 ): ?>
	                ...
	                <?php
	                // Date
	                if (valen_themeoption('show_date', 1) == 1) :
	            ...
	            <h3 class="post-title">
	              <a href="<?php the_permalink(); ?>" title="<?php echo esc_attr( sprintf( esc_html__( 'Permalink to %s', 'valen' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark"><?php the_title(); ?></a>
	            </h3>
            </div>
        </div>
	</article>
