<?php
if ( ! class_exists( 'ValenCompileSass' ) ) {
	class ValenCompileSass {
		function valen_getstyle($compile = 2, $scss = array('dir' => '', 'name' => ''), $css = array('dir' => '', 'name' => ''), $format = 'scss_formatter_compressed', $variables = array() ) {
			...
			if( $compile == 2 || !file_exists(get_template_directory() . '/assets/css/' . $css['name'] . '.css') )
				$this->valen_compilescss($scss, $css, $format, $scss_variables);
			return $css['name'] . '.css';
		}
		function valen_compilescss($scss, $css, $format, $scss_variables) {
