...
// Variables
	...
	$cache_key   = Icelander_Menu::get_cache_key_social();
	$cache_group = 'icelander_' . get_bloginfo( 'language' );

	$back_to_top  = '<li class="back-to-top-link">';
	$back_to_top .= '<a href="#page" class="back-to-top animated" title="' . esc_attr__( 'Back to top', 'icelander' ) . '">';
	$back_to_top .= '<span class="screen-reader-text">' . esc_html__( 'Back to top &uarr;', 'icelander' ) . '</span>';
	$back_to_top .= '</a>';
	$back_to_top .= '</li>';

	$social_menu_html = wp_cache_get( $cache_key, $cache_group );
	$social_menu_args = Icelander_Menu::get_menu_args_social( '<ul data-id="%1$s" class="%2$s">%3$s' . $back_to_top . '</ul>' );


?>

<nav class="social-links" aria-label="<?php esc_attr_e( 'Social Menu', 'icelander' ); ?>">
	...
	if ( is_customize_preview() ) {
		 ...
		 *
		 * @see  Icelander_Menu::social_cache_flush()
		 */
