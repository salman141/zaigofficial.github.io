...
// Helper variables
	...
	$pagination_suffix = Icelander_Library::get_the_paginated_suffix( 'small' );
	...
	$intro_title_tag = apply_filters( 'wmhook_icelander_intro_title_tag', $intro_title_tag, $post_id );
	$title           = apply_filters( 'wmhook_icelander_intro_title', $title, $post_id );
...
// Processing
	...
	if ( ! $pagination_suffix ) {
		...
		// Page summary setup
			...
			add_filter( 'wmhook_icelander_summary_continue_reading_pre', '__return_empty_string' );
			...
			remove_filter( 'wmhook_icelander_summary_continue_reading_pre', '__return_empty_string' );
...
// Output
	 ...
	 */
		...
		if ( 'h3' === $intro_title_tag ) {
			$title = '<h2 class="screen-reader-text">' . esc_html__( 'Introduction', 'icelander' ) . '</h2>' . $title;
		}
	...
	if ( $page_summary ) :
		...
		echo apply_filters( 'wmhook_icelander_intro_excerpt', $page_summary, $post_id );
