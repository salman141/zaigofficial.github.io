 ...
 * 100) Helpers
 */
final class Icelander_Library_CSS_Variables {
	 ...
	 */
		...
		public static $cache_key = 'icelander_css_vars';
			...
			// Processing
				...
				// Hooks
					...
					// Actions
						...
						add_action( 'wmhook_icelander_library_theme_upgrade', __CLASS__ . '::cache_flush' );
	 ...
	 */
		...
		public static function get_variables_array() {
			...
			// Processing
				...
				foreach ( (array) apply_filters( 'wmhook_icelander_theme_options', array() ) as $option ) {
					...
					if (
						'Icelander_Library_Sanitize::css_fonts' === $option['css_var']
					...
					// RGBA colors.
					$rgba_alphas = (array) apply_filters( 'wmhook_icelander_css_rgb_alphas', array() );
					if ( isset( $rgba_alphas[ $option['id'] ] ) ) {
						...
						foreach ( $alphas as $alpha ) {
							...
							$css_vars[ '--' . $option['id'] . '--a0' ] = self::color_hex_to_rgba( $value, 0 );
						}
					}
	 ...
	 */
		...
		public static function compatibility() {
			...
			// Processing
				...
				wp_enqueue_script(
					'css-vars-ponyfill',
					get_theme_file_uri( ICELANDER_LIBRARY_DIR . 'js/vendor/css-vars-ponyfill/css-vars-ponyfill.min.js' ),
					array(),
					'1.16.1'
				);
...
} // /Icelander_Library_CSS_Variables

add_action( 'after_setup_theme', 'Icelander_Library_CSS_Variables::init', 20 );
