 ...
 * 20) Customizer core
 */
final class Icelander_Library_Customize {
	 ...
	 */
		...
		public static function assets() {
			...
			// Processing
				...
				// Styles
					...
					wp_enqueue_style(
							'icelander-customize-controls',
							get_theme_file_uri( ICELANDER_LIBRARY_DIR . 'css/customize.css' ),
							false,
					...
					// RTL setup
						...
						wp_style_add_data( 'icelander-customize-controls', 'rtl', 'replace' );

				// Scripts

					wp_enqueue_script(
							'icelander-customize-controls',
							get_theme_file_uri( ICELANDER_LIBRARY_DIR . 'js/customize-controls.js' ),
		...
		public static function preview_scripts() {
			...
			// Pre
				...
				$pre = apply_filters( 'wmhook_icelander_library_customize_preview_scripts_pre', false );
			...
			// Helper variables
				...
				$theme_options = apply_filters( 'wmhook_icelander_theme_options', array() );
				...
				if (
					...
					foreach ( $theme_options as $theme_option ) {
						...
						if (
							...
							$output_single .= "\t" . '}'. "\r\n";
							$output_single .= ');'. "\r\n";
							$output_single  = apply_filters( 'wmhook_icelander_library_customize_preview_scripts_option_' . $theme_option['id'], $output_single );
			...
			// Output
				...
				if ( $output = trim( $output ) ) {
					echo apply_filters( 'wmhook_icelander_library_customize_preview_scripts_output', '<!-- Theme custom scripts -->' . "\r\n" . '<script type="text/javascript"><!--' . "\r\n" . '( function( $ ) {' . "\r\n\r\n" . trim( $output ) . "\r\n\r\n" . '} )( jQuery );' . "\r\n" . '//--></script>' );
				}
	 ...
	 */
		...
		public static function customize( $wp_customize ) {
			...
			// Pre
				...
				$pre = apply_filters( 'wmhook_icelander_library_customize_pre', false, $wp_customize );
			...
			// Helper variables
				...
				$theme_options = (array) apply_filters( 'wmhook_icelander_theme_options', array() );

				ksort( $theme_options );

				$allowed_option_types = apply_filters( 'wmhook_icelander_library_customize_allowed_option_types', array(
					'checkbox',
					'color',
					'email',
					'hidden',
				...
				// To make sure our customizer sections start after WordPress default ones
					...
					$priority = absint( apply_filters( 'wmhook_icelander_library_customize_priority', 0 ) );

				// Default section name in case not set (should be overwritten anyway)

					$customizer_panel   = '';
					$customizer_section = 'icelander';
			...
			// Processing
				...
				// Custom controls
					...
					require_once ICELANDER_LIBRARY . 'includes/classes/class-customize-control-hidden.php';
					require_once ICELANDER_LIBRARY . 'includes/classes/class-customize-control-html.php';
					require_once ICELANDER_LIBRARY . 'includes/classes/class-customize-control-multiselect.php';
					require_once ICELANDER_LIBRARY . 'includes/classes/class-customize-control-radio-matrix.php';
					require_once ICELANDER_LIBRARY . 'includes/classes/class-customize-control-select.php';

					do_action( 'wmhook_icelander_library_customize_load_controls', $wp_customize );
						...
						foreach ( $theme_options as $theme_option ) {
							if (
								...
								if ( isset( $theme_option['in_panel'] ) ) {
									...
									$panel_type = apply_filters( 'wmhook_icelander_library_customize_panel_type', $panel_type, $theme_option, $theme_options );
									$panel_id   = apply_filters( 'wmhook_icelander_library_customize_panel_id', $panel_id, $theme_option, $theme_options );
								...
								switch ( $theme_option['type'] ) {
									...
									case 'radio':
										$wp_customize->add_setting(
											...
											array(
												...
												'default'              => $default,
												'transport'            => $transport,
												'sanitize_callback'    => ( 'checkbox' === $theme_option['type'] ) ? ( 'Icelander_Library_Sanitize::checkbox' ) : ( 'Icelander_Library_Sanitize::select' ),
												'sanitize_js_callback' => ( 'checkbox' === $theme_option['type'] ) ? ( 'Icelander_Library_Sanitize::checkbox' ) : ( 'Icelander_Library_Sanitize::select' ),
											)
										);
									...
									case 'multiselect':
										$wp_customize->add_setting(
											...
											array(
												...
												'default'              => $default,
												'transport'            => $transport,
												'sanitize_callback'    => ( isset( $theme_option['validate'] ) ) ? ( $theme_option['validate'] ) : ( 'Icelander_Library_Sanitize::multi_array' ),
												'sanitize_js_callback' => ( isset( $theme_option['validate'] ) ) ? ( $theme_option['validate'] ) : ( 'Icelander_Library_Sanitize::multi_array' ),
											)
										);
										$wp_customize->add_control( new Icelander_Customize_Control_Multiselect(
											$wp_customize,
											$option_id,
											array_merge( $generic, array(
												'choices' => ( isset( $theme_option['choices'] ) ) ? ( $theme_option['choices'] ) : ( '' ),
									...
									case 'hidden':
										...
										$wp_customize->add_control( new Icelander_Customize_Control_Hidden(
											$wp_customize,
											$option_id,
											array(
												'label'    => 'HIDDEN FIELD',
									...
									case 'html':
										...
										$wp_customize->add_control( new Icelander_Customize_Control_HTML(
											$wp_customize,
											$option_id,
											array(
												'label'           => ( isset( $theme_option['label'] ) ) ? ( $theme_option['label'] ) : ( '' ),
									...
									case 'radiomatrix':
										...
										$wp_customize->add_control( new Icelander_Customize_Control_Radio_Matrix(
											$wp_customize,
											$option_id,
											array_merge( $generic, array(
												'choices' => ( isset( $theme_option['choices'] ) ) ? ( $theme_option['choices'] ) : ( '' ),
									...
									case 'select':
										$wp_customize->add_setting(
											...
											array(
												...
												'default'              => $default,
												'transport'            => $transport,
												'sanitize_callback'    => 'Icelander_Library_Sanitize::select',
												'sanitize_js_callback' => 'Icelander_Library_Sanitize::select',
											)
										);
										$wp_customize->add_control( new Icelander_Customize_Control_Select(
											$wp_customize,
											$option_id,
											array_merge( $generic, array(
												'choices' => ( isset( $theme_option['choices'] ) ) ? ( $theme_option['choices'] ) : ( '' ),
...
} // /Icelander_Library_Customize

add_action( 'after_setup_theme', 'Icelander_Library_Customize::init', 20 );
