			...
			// Processing
				...
				// Hooks
					...
					// Filters
						...
						add_filter( 'the_excerpt', 'Icelander_Library::remove_shortcodes' );
						add_filter( 'the_excerpt', __CLASS__ . '::excerpt', 20 );

						add_filter( 'wmhook_icelander_intro_excerpt', __CLASS__ . '::excerpt', 20 );
	 ...
	 */
		...
		public static function excerpt( $excerpt = '' ) {
			...
			// Requirements check
				...
				if ( post_password_required( $post_id ) ) {
					if ( ! is_single( $post_id ) ) {
						return esc_html__( 'This content is password protected.', 'icelander' )
						       . ' <a href="' . esc_url( get_permalink() ) . '">'
						       . esc_html__( 'Enter the password to view it.', 'icelander' )
						       . '</a>';
					}
					return;
				}
			...
			// Processing
				...
				if (
					! is_single( $post_id )
					&& Icelander_Library::has_more_tag()
				) {
	 ...
	 */
		...
		public static function continue_reading( $post_excerpt = '', $post = null ) {
			...
			// Processing
				...
				if (
					...
					&& ! Icelander_Post::is_singular( $post_id )
					&& ! Icelander_Library::has_more_tag()
					&& in_array(
						get_post_type( $post_id ),
						(array) apply_filters( 'wmhook_icelander_summary_continue_reading_post_type', array( 'post', 'page' ) )
					)
				) {
		...
		public static function get_continue_reading_html( $post = null, $scope = '' ) {
			...
			// Pre
				...
				$pre = apply_filters( 'wmhook_icelander_summary_continue_reading_pre', false, $post, $scope );
			...
			// Output
				...
				return (string) apply_filters( 'wmhook_icelander_summary_continue_reading', $html, $post, $scope );
...
} // /Icelander_Post_Summary

add_action( 'after_setup_theme', 'Icelander_Post_Summary::init' );
