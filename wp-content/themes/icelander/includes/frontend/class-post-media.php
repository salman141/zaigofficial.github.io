			...
			// Processing
				...
				// Hooks
					...
					// Filters
						...
						add_filter( 'wmhook_icelander_post_media_pre', __CLASS__ . '::media_disable' );

						add_filter( 'wmhook_icelander_post_media_image_size', __CLASS__ . '::size' );
						add_filter( 'wmhook_icelander_post_media_image_size', __CLASS__ . '::size_masonry' );
		...
		public static function size( $image_size ) {
			...
			// Processing
				...
				if ( is_attachment() ) {
					...
					$image_size = 'icelander-thumbnail';
				}
	 ...
	 */
		...
		public static function media( $args = array() ) {
			...
			// Pre
				...
				$pre = apply_filters( 'wmhook_icelander_post_media_pre', false, $args );
			...
			// Helper variables
				...
				$output     = apply_filters( 'wmhook_icelander_post_media_output_pre', '', $args );
				$image_size = apply_filters( 'wmhook_icelander_post_media_image_size', 'thumbnail', $args );
				$class      = 'entry-media';
		...
		public static function media_disable( $pre ) {
			...
			// Processing
				...
				if ( Icelander_Post::is_singular() && ! is_attachment() ) {
					$pre = '';
				}
	 ...
	 */
		...
		public static function image_featured( $image_size ) {
			...
			// Pre
				...
				$pre = apply_filters( 'wmhook_icelander_post_media_image_featured_pre', false, $image_size );
			...
			// Processing
				...
				if (
					...
					$image_link = array_filter( (array) apply_filters( 'wmhook_icelander_post_media_image_featured_link', $image_link ) );
...
} // /Icelander_Post_Media

add_action( 'after_setup_theme', 'Icelander_Post_Media::init' );
