			...
			// Processing
				...
				// Hooks
					...
					// Actions
						...
						add_action( 'wmhook_icelander_site_info_after', __CLASS__ . '::social' );

						add_action( 'tha_footer_top', __CLASS__ . '::footer' );

						add_action( 'wmhook_icelander_menu_footer_after', __CLASS__ . '::footer_search' );

						add_action( 'wp_update_nav_menu',   __CLASS__ . '::social_cache_flush' );
						add_action( 'customize_save_after', __CLASS__ . '::social_cache_flush' );
						add_action( 'wmhook_icelander_library_theme_upgrade', __CLASS__ . '::social_cache_flush' );

					// Filters

						add_filter( 'wmhook_icelander_svg_get_social_icons', __CLASS__ . '::social_links_icons' );
		...
		public static function register() {
			...
			// Processing
				...
				register_nav_menus( array(
						'primary'   => esc_html_x( 'Primary', 'Navigational menu location', 'icelander' ),
						'secondary' => esc_html_x( 'Secondary', 'Navigational menu location', 'icelander' ),
						'footer'    => esc_html_x( 'Footer', 'Navigational menu location', 'icelander' ),
						'social'    => esc_html_x( 'Social Links', 'Navigational menu location', 'icelander' ),
					) );
				...
				// Processing
					...
					if ( ! $fallback ) {
						...
						// For `wp_nav_menu()`
							...
							$args['depth']           = 4;
							$args['fallback_cb']     = 'Icelander_Menu::primary_fallback';
							$args['items_wrap']      = '<ul id="menu-primary" class="menu-primary">%3$s<li class="menu-toggle-skip-link-container"><a href="#menu-toggle" class="menu-toggle-skip-link">' . esc_html__( 'Skip to menu toggle button', 'icelander' ) . '</a></li></ul>';
						...
						// For `wp_page_menu()`
							...
							$args['after']  = '<li class="menu-toggle-skip-link-container"><a href="#menu-toggle" class="menu-toggle-skip-link">' . esc_html__( 'Skip to menu toggle button', 'icelander' ) . '</a></li></ul>';
		...
		public static function nav_menu_item_expander( $item_output, $item, $depth, $args ) {
			...
			// Processing
				...
				if (
					...
					$item_output = str_replace(
						$args->link_after . '</a>',
						$args->link_after . ' <span class="expander" aria-label="' . esc_attr__( '(Focus the link to toggle submenu.)', 'icelander' ) . '"></span></a>',
						$item_output
					);
		 ...
		 */
			...
			public static function nav_menu_social_icons( $item_output, $item, $depth, $args ) {
				...
				// Variables
					...
					$social_icons = Icelander_SVG::get_social_icons();
					$social_icon  = 'chain';
			...
			public static function get_cache_key_social() {
				...
				// Output
					...
					return apply_filters(
						'wmhook_icelander_get_cache_key',
						'icelander_social_links',
						'menu-social'
					);
			...
			public static function social_cache_flush() {
				...
				// Processing
					...
					wp_cache_delete(
						...
						'icelander_' . get_bloginfo( 'language' )
					);
...
add_action( 'after_setup_theme', 'Icelander_Menu::init' );
