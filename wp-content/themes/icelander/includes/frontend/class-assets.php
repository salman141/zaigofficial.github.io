			...
			// Processing
				...
				// Hooks
					...
					// Filters
						...
						add_filter( 'wmhook_icelander_setup_editor_stylesheets', __CLASS__ . '::editor_stylesheets' );
		...
		public static function register_styles() {
			...
			// Helper variables
				...
				$register_assets = array(
					...
					'icelander-google-fonts' => array( 'src' => self::google_fonts_url() ),

					'icelander-stylesheet-main'       => array( 'src' => get_theme_file_uri( 'assets/css/main.css' ), 'rtl' => 'replace' ),
					'icelander-stylesheet-shortcodes' => array( 'src' => get_theme_file_uri( 'assets/css/shortcodes.css' ), 'rtl' => 'replace' ),
					'icelander-stylesheet-custom'     => array( 'src' => get_theme_file_uri( 'assets/css/custom-styles.css' ) ),

					'icelander-stylesheet-global' => array(
						'src'  => '',
						'deps' => array(
							'icelander-stylesheet-main',
							'icelander-stylesheet-shortcodes',
							'icelander-stylesheet-custom',
						)
					),

					'icelander-stylesheet-print' => array( 'src' => get_theme_file_uri( 'assets/css/print.css' ), 'media' => 'print', 'rtl' => 'replace' ),
				);

				$register_assets = (array) apply_filters( 'wmhook_icelander_assets_register_styles', $register_assets );
			...
			// Processing
				...
				foreach ( $register_assets as $handle => $atts ) {
					...
					$ver   = ( isset( $atts['ver'] ) ) ? ( $atts['ver'] ) : ( ICELANDER_THEME_VERSION );
		...
		public static function register_scripts() {
			...
			// Helper variables
				...
				$register_assets = array(
						...
						'icelander-skip-link-focus-fix' => array( 'src' => get_theme_file_uri( 'assets/js/skip-link-focus-fix.js' ), 'deps' => array() ),
						'icelander-scripts-global'      => array( 'src' => get_theme_file_uri( 'assets/js/scripts-global.js' ), 'deps' => $script_global_deps ),
						'icelander-scripts-masonry'     => array( 'src' => get_theme_file_uri( 'assets/js/scripts-masonry.js' ), 'deps' => array( 'jquery-masonry' ) ),
						'icelander-scripts-nav-a11y'    => array( get_theme_file_uri( 'assets/js/scripts-navigation-accessibility.js' ) ),
						'icelander-scripts-nav-mobile'  => array( get_theme_file_uri( 'assets/js/scripts-navigation-mobile.js' ) ),
					);

				$register_assets = (array) apply_filters( 'wmhook_icelander_assets_register_scripts', $register_assets );
	 ...
	 */
			...
			// Processing
				...
				// Google Fonts
					...
					if ( self::google_fonts_url() ) {
						$enqueue_assets[0] = 'icelander-google-fonts';
					}
				...
				// Main
					...
					$enqueue_assets[10] = 'icelander-stylesheet-global';

				// Print

					$enqueue_assets[100] = 'icelander-stylesheet-print';

				// Filter enqueue array

					$enqueue_assets = (array) apply_filters( 'wmhook_icelander_assets_enqueue_styles', $enqueue_assets );
		...
		public static function enqueue_scripts() {
			...
			// Helper variables
				...
				$body_classes = (array) Icelander_Header::body_class();

				$breakpoints = (array) apply_filters( 'wmhook_icelander_assets_enqueue_scripts_breakpoints', array(
						's'     => 448,
						'm'     => 672,
						'l'     => 880,
						'xl'    => 1280,
			...
			// Processing
				...
				// Skip link focus fix
					...
					$enqueue_assets[10] = 'icelander-skip-link-focus-fix';

				// Navigation scripts

					if ( Icelander_Header::is_enabled() ) {
						$enqueue_assets[20] = 'icelander-scripts-nav-a11y';

						if ( get_theme_mod( 'navigation_mobile', true ) ) {
							$enqueue_assets[25] = 'icelander-scripts-nav-mobile';
						}
					}
				...
				// Masonry
					...
					if ( (bool) apply_filters( 'wmhook_icelander_is_masonry_layout', false ) ) {
						$enqueue_assets[40] = 'icelander-scripts-masonry';
					}

				// Global theme scripts

					$enqueue_assets[100] = 'icelander-scripts-global';

				// Filter enqueue array

					$enqueue_assets = (array) apply_filters( 'wmhook_icelander_assets_enqueue_scripts', $enqueue_assets );

				// Pass CSS breakpoint into JS (from `assets/scss/_setup.scss`)

					if ( ! empty( $breakpoints ) ) {
						wp_localize_script(
								'icelander-skip-link-focus-fix',
								'$icelanderBreakpoints',
								$breakpoints
							);
					}
		 ...
		 * Use the `icelander` handle in `wp_add_inline_style`.
	 ...
	 */
		...
		public static function google_fonts_url( $fonts = array() ) {
			...
			// Pre
				...
				$pre = apply_filters( 'wmhook_icelander_assets_google_fonts_url_pre', false, $fonts );
			...
			// Variables
				...
				$subset = (array) apply_filters( 'wmhook_icelander_assets_google_fonts_url_subset', $subset );

				$fonts_setup = array_filter( (array) apply_filters( 'wmhook_icelander_assets_google_fonts_url_fonts_setup', array() ) );

				if ( empty( $fonts_setup ) && ! empty( $fonts ) ) {
					$fonts_setup = (array) $fonts;
				}

				$url_base = Icelander_Library::fix_ssl_urls( 'http://fonts.googleapis.com/css' );
	 ...
	 */
		...
		public static function editor_stylesheets() {
			...
			// Processing
				...
				// Filter and order
					...
					$visual_editor_stylesheets = (array) apply_filters( 'wmhook_icelander_assets_editor', $visual_editor_stylesheets );
		...
		public static function editor_frontend_stylesheets( $stylesheets ) {
			...
			// Output
				...
				return array_merge(
					(array) $stylesheets,
					array_filter( (array) apply_filters( 'wmhook_icelander_setup_editor_stylesheets', array() ) )
				);
		...
		public static function resource_hints( $urls, $relation_type ) {
			...
			// Variables
				...
				$url_fonts = Icelander_Library::fix_ssl_urls( 'http://fonts.gstatic.com' );


			// Processing

				if (
						wp_style_is( 'icelander-google-fonts', 'queue' )
...
add_action( 'after_setup_theme', 'Icelander_Assets::init' );
