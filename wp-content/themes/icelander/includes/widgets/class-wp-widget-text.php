 ...
 */
	 ...
	 * 40) Icon fallback
	 */
	class Icelander_WP_Widget_Text extends WP_Widget_Text {
				...
				// Processing
					...
					if ( is_numeric( $widget_media['image'] ) ) {
						...
						$output .= '<img
							...
							alt="' . esc_attr__( 'Widget featured image', 'icelander' ) . '"
							/>';
					}
				...
				// Output
					...
					<p class="text-widget-media-icon">
						<label for="<?php echo esc_attr( $this->get_field_id( 'icon' ) ); ?>">
							<strong><?php esc_html_e( 'Set icon CSS class:', 'icelander' ); ?></strong><br>
							<span class="description" style="display: inline-block; padding: 0 0 .38em">
								<em>
									<?php

									printf(
										esc_html__( 'For displaying icons on your website use a plugin, such as %1$s or %2$s.', 'icelander' ),
										'<a href="https://wordpress.org/plugins/better-font-awesome/">' . esc_html_x( 'Better Font Awesome', 'Plugin name.', 'icelander' ) . '</a>',
										'<a href="https://wordpress.org/plugins/ionicons-official/">' . esc_html_x( 'Ionicons Official', 'Plugin name.', 'icelander' ) . '</a>'
									);

									if ( class_exists( 'WM_Icons' ) ) {
										echo '<br><strong>' . esc_html__( 'As your theme supports custom icons, you can simply use icon classes from Appearance &rarr; Icon Font.', 'icelander' ) . '</strong>';
									}
				...
				// Output
					...
					<p class="text-widget-media-image">
						<label for="<?php echo esc_attr( $this->get_field_id( 'image' ) ); ?>">
							<strong><?php esc_html_e( 'Set image:', 'icelander' ); ?></strong><br>
							<span class="description" style="display: inline-block; padding: 0 0 .38em">
								<em>
									<?php esc_html_e( 'Choose a featured image for this widget.', 'icelander' ); ?>
								</em>
							</span>
						</label>
						<br>
						<button class="button button-hero text-widget-media-image-select"><?php esc_html_e( 'Select image', 'icelander' ); ?></button>
						...
						<span class="text-widget-media-image-preview"<?php if ( empty( $instance['image'] ) ) { echo ' style="display: none;"'; } ?>>
							<img src="<?php
								...
								?>" alt="<?php esc_attr_e( 'Widget featured image', 'icelander' ); ?>" />
							<button class="button text-widget-media-image-remove">
								<span class="screen-reader-text"><?php esc_html_e( 'Remove image', 'icelander' ); ?></span>
							</button>
						</span>
					</p>
		 ...
		 */
			...
			public static function style_icon_fallback() {
				...
				// Output
					...
					echo '<style id="icelander-text-widget-icon-fallback">'
	 ...
	 * @version  1.0.0
	 */
	function icelander_register_widget_text() {

		// Processing

			unregister_widget( 'WP_Widget_Text' );

			register_widget( 'Icelander_WP_Widget_Text' );

	} // /icelander_register_widget_text

	add_action( 'widgets_init', 'icelander_register_widget_text' );
