	 ...
	 */
		...
		public static function init() {
			...
			// Variables
				...
				$post_types        = array_filter( (array) apply_filters( 'wmhook_icelander_subtitles_post_types', array( 'post', 'page' ) ) );
			...
			// Processing
				...
				// Hooks
					...
					// Actions
						...
						add_action( 'wmhook_icelander_widget_recent_posts_before', __CLASS__ . '::force_subtitle_add', 0 );
						add_action( 'wmhook_icelander_widget_recent_posts_after', __CLASS__ . '::force_subtitle_remove', 0 );

					// Filters

						add_filter( 'the_title', __CLASS__ . '::subtitle', 10, 2 );
						add_filter( 'single_post_title', __CLASS__ . '::subtitle', 100, 2 );
						add_filter( 'wmhook_icelander_intro_title', __CLASS__ . '::subtitle', 10, 2 );
		...
		public static function subtitle( $title, $post ) {
			...
			// Processing
				...
				if (
					...
					&& (bool) apply_filters( 'wmhook_icelander_subtitle_enabled', in_the_loop() )
				) {
	 ...
	 */
		...
		public static function force_subtitle_add() {
			...
			// Processing
				...
				add_filter( 'wmhook_icelander_subtitle_enabled', '__return_true' );
		...
		public static function force_subtitle_remove() {
			...
			// Processing
				...
				remove_filter( 'wmhook_icelander_subtitle_enabled', '__return_true' );
...
add_action( 'after_setup_theme', 'Icelander_Subtitles::init', 20 );
