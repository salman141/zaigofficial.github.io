 ...
 * 30) Options
 */
class Icelander_Breadcrumb_NavXT {
		...
		public static function options( $options = array() ) {
			...
			// Processing
				...
				$options[ 300 . 'layout' . 600 ] = array(
						'type'    => 'html',
						'content' => '<h3>' . esc_html__( 'Breadcrumbs', 'icelander' ) . '</h3>',
					);

				$options[ 300 . 'layout' . 610 ] = array(
						'type'    => 'radio',
						'id'      => 'breadcrumbs_display',
						'label'   => esc_html__( 'Breadcrumbs display', 'icelander' ),
						'choices' => array(
								'before' => esc_html__( 'Before content only', 'icelander' ),
								'both'   => esc_html__( 'Both before and after content', 'icelander' ),
								'after'  => esc_html__( 'After content only', 'icelander' ),
							),
						'default' => 'after',
					);
...
} // /Icelander_Breadcrumb_NavXT

add_action( 'after_setup_theme', 'Icelander_Breadcrumb_NavXT::init' );
