 ...
 * 50) Custom fields
 */
class Icelander_WooCommerce_Pages {
			...
			// Processing
				...
				// Hooks
					...
					// Filters
						...
						add_filter( 'wmhook_icelander_intro_title', __CLASS__ . '::page_endpoint_title' );

						add_filter( 'the_content', __CLASS__ . '::page_title_replace' );

						add_filter( 'woocommerce_cross_sells_total', __CLASS__ . '::cross_sells_total' );

						add_filter( 'wmhook_icelander_woocommerce_checkout_guide', __CLASS__ . '::guide_steps', 10, 2 );

						add_filter( 'wmhook_icelander_acf_field_group', __CLASS__ . '::acf_modify', 10, 3 );
		...
		public static function shop_page_templates( $page_templates ) {
			...
			// Processing
				...
				if ( get_the_ID() === wc_get_page_id( 'shop' ) ) {
					 ...
					 */
					$page_templates['templates/no-intro.php'] = esc_html__( 'No intro', 'icelander' );
	 ...
	 */
		...
		public static function cart_products_list_title() {
			...
			// Output
				...
				echo '<h2 class="cart-table-title">'
				     . esc_html__( 'Cart content', 'icelander' )
				     . '<small class="cart-table-products-count"> '
				     . sprintf(
				     		esc_html( _nx( '(%d item)', '(%d items)', absint( wp_strip_all_tags( WC()->cart->get_cart_contents_count() ) ), 'Shopping cart items count.', 'icelander' ) ),
		...
		public static function coupon_description() {
			...
			// Output
				...
				echo '<p class="description coupon-description">' . esc_html__( 'Use your discount coupon code here', 'icelander' ) . '</p>';
		...
		public static function cross_sells_total( $total ) {
			...
			// Output
				...
				return Icelander_WooCommerce_Helpers::return_number( 'cross_sells_total' );
		...
		public static function button_continue_shopping() {
			...
			// Output
				...
				echo '<a href="' . esc_url( wc_get_page_permalink( 'shop' ) ) . '" class="button button-continue-shopping">' . esc_html__( 'Continue Shopping', 'icelander' ) . '</a>';
	 ...
	 */
		...
		public static function checkout_title() {
			...
			// Output
				...
				echo '<h2 class="screen-reader-text">' . esc_html__( 'Checkout', 'icelander' ) . '</h2>';
		...
		public static function guide( $template_name = '' ) {
			...
			// Helper variables
				...
				$steps = (array) apply_filters( 'wmhook_icelander_woocommerce_checkout_guide', array(), $template_name );
		...
		public static function guide_steps( $steps = array(), $template_name = '' ) {
			...
			// Processing
				...
				$steps[ 10 . 'cart' ] = array(
						'title'    => esc_html__( 'Shopping Cart', 'icelander' ),
				...
				$steps[ 20 . 'checkout' ] = array(
						'title'    => esc_html__( 'Checkout', 'icelander' ),
				...
				$steps[ 30 . 'completed' ] = array(
						'title'    => esc_html__( 'Order Completed', 'icelander' ),
						'template' => array(
				...
				if ( ! empty( $wp->query_vars['order-pay'] ) ) {
					...
					$steps[ 25 . 'order-pay' ] = array(
							'title'          => esc_html__( 'Pay for Order', 'icelander' ),
...
} // /Icelander_WooCommerce_Pages

add_action( 'after_setup_theme', 'Icelander_WooCommerce_Pages::init' );
