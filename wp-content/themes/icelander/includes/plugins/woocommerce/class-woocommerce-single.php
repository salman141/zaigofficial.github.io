 ...
 * 100) Others
 */
class Icelander_WooCommerce_Single {
	 ...
	 */
		...
		public static function product_thumbnails_columns( $columns ) {
			...
			// Output
				...
				return Icelander_WooCommerce_Helpers::return_number( 'thumbnails_columns' );
	 ...
	 */
		...
		public static function comment_pagination_args( $args = array() ) {
			...
			// Processing
				...
				$args['prev_text'] = esc_html_x( '&laquo;', 'Pagination text (visible): previous.', 'icelander' ) . '<span class="screen-reader-text"> ' . esc_html_x( 'Previous page', 'Pagination text (hidden): previous.', 'icelander' ) . '</span>';
				$args['next_text'] = '<span class="screen-reader-text">' . esc_html_x( 'Next page', 'Pagination text (hidden): next.', 'icelander' ) . ' </span>' . esc_html_x( '&raquo;', 'Pagination text (visible): next.', 'icelander' );
				$args['type']      = 'plain';


			// Output

				return (array) apply_filters( 'wmhook_icelander_pagination_args', $args, 'woocommerce-comments' );
	 ...
	 */
		...
		public static function products_list_args( $args ) {
			...
			// Processing
				...
				$args['posts_per_page'] = $args['columns'] = Icelander_WooCommerce_Helpers::return_number( 'shop_columns' );
		...
		public static function read_more_link( $excerpt ) {
			...
			// Processing
				...
				if (
					...
					$excerpt .= '<p class="product-description-link-container"><a href="#tab-description" class="product-description-link">' . esc_html__( 'More details&hellip;', 'icelander' ) . '</a></p>';
				}
...
} // /Icelander_WooCommerce_Single

add_action( 'after_setup_theme', 'Icelander_WooCommerce_Single::init' );
