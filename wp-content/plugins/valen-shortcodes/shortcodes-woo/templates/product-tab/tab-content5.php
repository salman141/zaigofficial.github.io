...
if( $loop->have_posts() ) :
    ?>
    <div class="first-item product_list grid style<?php echo $_gridstyle; ?>">
    <?php 
    $i = 0;
    while ( $loop->have_posts() ) : $loop->the_post();
        wc_get_template( 'vc/item-grid.php', array('class' => $_class, 'gridstyle' => $_gridstyle, 'thumb' => 'big') );
        $i++;
        if ( $i == 1 ) break;
    endwhile; ?>
    </div>
